"""NEC IR receiver/transmitter for Raspberry Pi Pico MicroPython."""
...
class NECTransmitter:
    ...
    def send(self, address, command):
        ...
        for bit_index in range(16):
            self._bit((address >> bit_index) & 1)
        for bit_index in range(8):
            self._bit((command >> bit_index) & 1)

        inverted_command = command ^ 0xFF
        for bit_index in range(8):
            self._bit((inverted_command >> bit_index) & 1)
...
class NECReceiver:
    ...
    def _on_edge(self, pin):
        now = ticks_us()
        new_level = pin.value()
        previous_level = 0 if new_level else 1

        if self._count < self.MAX_EDGES:
            self._levels[self._count] = previous_level
    ...
    def read(self):
        """Return ``(address, command)`` once a complete NEC frame is idle."""
        if self._count < 66:
            return None
        if ticks_diff(ticks_us(), self._last_edge_us) < self.FRAME_IDLE_US:
            return None
